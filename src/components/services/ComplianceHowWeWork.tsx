...
import { REGULATION_COMPLIANCE_HOW_WE_WORK } from "@/data/site";
...
export default function ComplianceHowWeWork() {
  const timelineRef = useRef<HTMLDivElement>(null);
  const [path, setPath] = useState<PathLayout | null>(null);
  ...
  useEffect(() => {
    ...
    const sync = () => {
      const dots = Array.from(
        timeline.querySelectorAll<HTMLElement>(".rcwork-axis-dot"),
      );
      if (dots.length < 2) {
        setPath(null);
        return;
      ...
      <div className="rcwork-shell">
        ...
        <div className="rcwork-timeline" ref={timelineRef}>
          ...
          <ol className="rcwork-steps">
            {REGULATION_COMPLIANCE_HOW_WE_WORK.map((step, index) => {
